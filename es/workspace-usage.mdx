...
Devuelve el **historial de ejecuciones de agentes** de un workspace. Los resultados combinan ejecuciones actuales, filas archivadas y chats editados (consulta **Campos de cada elemento**). El orden es por `created_at` descendente y, a continuación, `id` descendente.
...
| :------------ | :------- | :------------ | :-------------- |
| Authorization | string | Sí | Token `Bearer` (clave API). El token debe incluir la ability **`use_agents`**. |
| x-workspace-id | integer | Sí | ID numérico del workspace. Valores inválidos o ausentes devuelven **422**. Debes pertenecer a ese workspace; si no, **403**. |
| Accept | string | No | Usa `application/json` si el cliente debe declarar JSON explícitamente. |

### **Parámetros de consulta**

| **Parámetro** | **Tipo** | **Requerido** | **Descripción** | **Predeterminado / reglas** |
| :------------ | :------- | :------------ | :-------------- | :------------------------- |
| range | string | No | Ventana relativa: `1d`, `7d` o `30d`. Se ignora si se envían `start_date` y `end_date`. | Si no se envía ninguna fecha, el valor efectivo por defecto es **`30d`**. |
| start_date | date (`YYYY-MM-DD`) | Condicional | Inicio del rango (inclusivo). | **Requerido con** `end_date`. |
| end_date | date (`YYYY-MM-DD`) | Condicional | Fin del rango (inclusivo). Debe ser `>= start_date`. | **Requerido con** `start_date`. El rango **no puede superar 90 días**; si no, **422**. |
| user_id | integer | No | Filtra por el usuario que ejecutó el agente. | Sin permiso para ver la actividad de otros en el workspace, solo ves **tus** ejecuciones; filtrar por otro usuario devuelve **403**. |
| type | string | No | Tipo de agente: `all`, `chat`, `image`, `text`, `voiceover`, `video`, `code`. | `all` u omitido = sin filtro de tipo. |
| page | integer | No | Número de página. | Predeterminado **1**, mínimo **1**. |
| per_page | integer | No | Tamaño de página. | Predeterminado **20**, entre **1** y **100**. |
