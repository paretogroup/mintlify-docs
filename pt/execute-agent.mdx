...
<ParamField body="temperature" type="string">
  Campo de Chat Agent. Temperatura de amostragem entre 0 e 2. Valores maiores geram resultados mais criativos (padrão: `"1"`).
</ParamField>
<ParamField body="model" type="string">
  Campo de Chat Agent. Identificador do modelo a ser utilizado (ex.: `"tess-6"`).
</ParamField>
<ParamField body="tools" type="string">
  Campo de Chat Agent. Configuração de ferramentas do agente (ex.: `"agent"`, `"no-tools"`).
</ParamField>
<ParamField body="root_id" type="integer">
  Campo de Chat Agent. ID de uma execução existente para continuar um thread de conversa.
</ParamField>
<ParamField body="messages" type="array">
  Campo de Chat Agent. As mensagens do agente. Obrigatório para modelos do tipo Chat. Suporta os papéis `user`, `assistant` e `developer`.
</ParamField>
