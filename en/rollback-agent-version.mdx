...
description: "Restores a specific agent version by creating a new version whose snapshot matches the target. Version history is preserved (no versions are deleted), and the rollback is recorded for audit. Your workspace role must allow rolling back agent versions."
