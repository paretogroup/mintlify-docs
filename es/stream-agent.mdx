---
title: "Ejecutar Agente en Stream"
description: "Ejecuta un agente de **chat** espec?fico por ID."
openapi: "api-reference/agents-stream.openapi.json POST /agents/{id}/execute"
---
...
| :--------------- | :------- | :------------ | :---------------------------------------------------------------------------------------------------------- |
| `x-workspace-id` | integer  | No            | ID del espacio de trabajo. Si no se proporciona, se usar? el espacio de trabajo seleccionado. Ser? requerido en una versi?n futura. |

### **Par?metros de Ruta**

| **Par?metro** | **Tipo** | **Requerido** | **Descripci?n**  |
| :------------ | :------- | :------------ | :--------------- |
| `id`          | integer  | S?            | El ID del agente. |

### **Cuerpo de la Solicitud**

| **Par?metro**       | **Tipo** | **Requerido** | **Descripci?n**                                                                                                              |
| :------------------ | :------- | :------------ | :--------------------------------------------------------------------------------------------------------------------------- |
| `stream`            | boolean  | S?            | Debe ser `true` para este endpoint de streaming.                                                                              |
| `temperature`       | string   | No            | Campo de Chat Agent. Temperatura de muestreo entre 0 y 2. Valores m?s altos generan resultados m?s creativos (pred.: `"1"`). |
| `model`             | string   | No            | Campo de Chat Agent. Identificador del modelo a utilizar (p. ej., `"tess-6"`).                                               |
| `tools`             | string   | No            | Campo de Chat Agent. Configuraci?n de herramientas del agente (p. ej., `"agent"`, `"no-tools"`).                             |
| `root_id`           | integer  | No            | Campo de Chat Agent. ID de una ejecuci?n existente para continuar un hilo de conversaci?n.                                   |
| `messages`          | array    | No            | Campo de Chat Agent. Los mensajes del agente. Admite los roles `user`, `assistant` y `developer`.                            |
| `file_ids`          | array    | No            | Array de IDs de archivos para adjuntar a la ejecuci?n.                                                                       |
| Otros campos en la ra?z | any | Depende del agente | Esto no es un nombre de campo fijo. Puede enviar otros campos requeridos por su agente directamente en la ra?z de la solicitud. Consulte qu? campos son obligatorios en [Obtener Agente por ID](https://docs.tess.im/es/api/endpoints/agents/get). |

### **Respuesta**

```json
data: {"id": 123, "status": "running", "output": "?Hola!", "error": null, "credits": null, "root_id": 123, "created_at": "2025-01-01T10:00:00.000000Z", "updated_at": "2025-01-01T10:00:00.000000Z", "template_id": 10}
