...
<ParamField body="temperature" type="string">
  Chat Agent field. Sampling temperature between 0 and 2. Higher values produce more creative outputs (default: `"1"`).
</ParamField>
<ParamField body="model" type="string">
  Chat Agent field. Model identifier to use for execution (e.g., `"tess-6"`).
</ParamField>
<ParamField body="tools" type="string">
  Chat Agent field. Tool configuration for the agent (e.g., `"agent"`, `"no-tools"`).
</ParamField>
<ParamField body="root_id" type="integer">
  Chat Agent field. ID of an existing execution to continue a conversation thread.
</ParamField>
<ParamField body="messages" type="array">
  Chat Agent field. The agent messages. Required for Chat Agent type templates. Supports `user`, `assistant`, and `developer` roles.
</ParamField>
<ParamField body="waitExecution" type="boolean">
  If `true`, waits for the execution to finish before returning (timeout: 100 s). For longer executions set `waitExecution: false` and poll the Get Agent Response endpoint (default: `false`).
</ParamField>
<ParamField body="file_ids" type="array">
  Array of file IDs to attach to the execution.
</ParamField>
