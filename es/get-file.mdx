...
description: "Recupera un archivo específico por su ID."
api: "GET https://api.tess.im/files/{fileId}"
authMethod: "bearer"
...
func main() {
    client := &http.Client{}
    req, err := http.NewRequest("GET", "https://api.tess.im/files/{fileId}", nil)
    if err != nil {
        fmt.Println(err)
        return
    }
...
class Program
{
    static async Task Main(string[] args)
    {
        using (var client = new HttpClient())
        {
            ...
            try
            {
                var response = await client.GetAsync("https://api.tess.im/files/{fileId}");
...
| **Parámetro** | **Tipo** | **Requerido** | **Descripción**   |
| :------------ | :------- | :------------ | :---------------- |
| fileId        | string   | Sí            | El ID del archivo |
