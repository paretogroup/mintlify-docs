...
description: "Ejecuta un agente específico por ID."
...
<ParamField body="temperature" type="string">
  Campo de Chat Agent. Temperatura de muestreo entre 0 y 2. Valores más altos generan resultados más creativos (predeterminado: `"1"`).
</ParamField>
<ParamField body="model" type="string">
  Campo de Chat Agent. Identificador del modelo a utilizar (p. ej., `"tess-6"`).
</ParamField>
<ParamField body="tools" type="string">
  Campo de Chat Agent. Configuración de herramientas del agente (p. ej., `"agent"`, `"no-tools"`).
</ParamField>
<ParamField body="root_id" type="integer">
  Campo de Chat Agent. ID de una ejecución existente para continuar un hilo de conversación.
</ParamField>
...
<ParamField body="waitExecution" type="boolean">
  Si es `true`, espera que la ejecución termine antes de retornar (timeout: 100 s). Para ejecuciones más largas, use `waitExecution: false` y haga polling en el endpoint Obtener Respuesta del Agente (predeterminado: `false`).
</ParamField>
<ParamField body="file_ids" type="array">
  Array de IDs de archivos para adjuntar a la ejecución.
</ParamField>
